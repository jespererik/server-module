...
def query_latest_node_name(conn):
    DB_LOGGER.debug('ENTER')
...
def query_nodes_by_name(conn, node_name):
    DB_LOGGER.debug('ENTER')
...
def query_sensors_by_name(conn, sensor_name):
    '''
        Executes a SELECT statement on the given database connection "conn" to query all entries of the "sensors" table
    ...
    try:
        ...
        sql = 'SELECT * FROM sensors WHERE name = ?'
        query_result = query_cursor.execute(sql, tokens).fetchall()
...
def query_node_id_by_name(conn, node_name):
    '''
        Executes a SELECT statement on the given database connection "conn" to query all entries of the "nodes" table
...
def query_sensor_id_by_name(conn, sensor_name, node_name):
    '''
        Executes a SELECT statement on the given database connection "conn" 
        to query all entries of the "sensors" table which
...
def query_readings_by_sensor_name(conn, sensor_name):
    DB_LOGGER.debug('ENTER')
    ...
    try:
        query_cursor = conn.cursor()
        DB_LOGGER.debug('\n => Querying for readings with sensor_id of sensor with name {}'.format(sensor_name))
...
def query_readings_by_sensor_name_and_readingtype(conn, sensor_name, reading_type):
    DB_LOGGER.debug('ENTER')
    tokens = (reading_type, sensor_name,)

    sql = 'SELECT data, timestamp FROM readings WHERE readingtype = ? AND sensor_id IN (SELECT id FROM sensors WHERE name = ?)'

    try:
        query_cursor = conn.cursor()
        DB_LOGGER.debug('\n => Querying for readings with readingtype {} and sensor_id of sensor with name {}'.format(reading_type, sensor_name))
...
def query_readings_by_sensor_id(conn, sensor_id):
    '''
        Executes a SELECT statement on the given database connection to query all entries of the readings table which
        ...
        Example usaage:
            query_readings_by_sensor_id(
                ...
                query_sensor_id_by_name('DHT11', 'Node#1')
            )
...
def query_readings_by_location(conn, node_location):
    '''
        Executes a SELECT statement on the given database connection to query all entries of the readings table which
...
def query_readings_by_location_and_readingtype(conn, node_location, reading_type):
    DB_LOGGER.debug("ENTER")
    tokens = (reading_type, node_location,)
    try:

        query_cursor = conn.cursor()

        DB_LOGGER.debug('\n=> Querying for readings with values (location: {} readingtype: {})'.format(node_location, reading_type))

        sql =  'SELECT data, timestamp FROM readings WHERE readingtype = ? AND sensor_id IN (\
                SELECT id FROM sensors WHERE node_id in (\
                        SELECT id FROM nodes WHERE location = ?)\
                    )'
...
if __name__ == '__main__':
    ...
    insert_sensor(DB_CONNECTION, ('DHT11', query_node_id_by_name(DB_CONNECTION, 'Node#1')))
    insert_sensor(DB_CONNECTION, ('DHT11', query_node_id_by_name(DB_CONNECTION, 'Node#2')))
    insert_sensor(DB_CONNECTION, ('DHT11', query_node_id_by_name(DB_CONNECTION, 'Node#3')))
    ...
    print(
        query_readings_by_location_and_readingtype(
            DB_CONNECTION,
            'outhouse',
            'temperature'
        )
    ...
    print(
        query_readings_by_sensor_name_and_readingtype(
            DB_CONNECTION,
            'DHT11',
            'temperature'
        )
